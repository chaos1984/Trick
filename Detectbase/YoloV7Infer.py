...
class ObjectDetect(Infer):
    def __init__(self,config):
        ...
        self.imgsize = check_img_size(self.imgsize[0], s=self.stride)
    ...
    def run(self, img,res={}):
        ...
        img0 = img.copy()
        #Load image
        img = letterbox(img, self.imgsize, stride=self.stride.cpu().numpy(), auto=True)[0]
        # img = LoadImages(source, img_size=imgsz, stride=stride)
...
class Validation(Infer):
    ...
    def run(self, rundir,save_dir):
        ...
        for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
            ...
            lb = []
            out = non_max_suppression(pred, self.conf_thres, self.iou_thres, self.classes, self.agnostic_nms)
            # Metrics
